...
<section id="hero" class="d-flex align-items-center" >
  ...
  <div class="card-header card-header-warning" style="background-color: #8a855b">
    ...
    <table class="table">
      ...
      <tbody   *ngFor="let fd of food" >
        <tr><td>{{fd.name}}</td>
          <td>{{fd.category}}</td>
          <td> $ {{fd.price}}</td>
          <td class="text-danger"> <span (click)="edit(fd)" >Edit</span> <span (click)="delete(fd._id)" >Delete</span></td>
        </tr>
...
export class AdminHomeComponent {
  ...
  delete(foodId: string) {
    this.service.deleteFood(foodId).subscribe((resp) => alert(resp.data));
